/* SponsorMohawk Visitor Intelligence + Contact Form */
(() => {
  'use strict';
  ...
  const sessionStartedAt = Number(sessionStorage.getItem('sm_session_started_at') || now);
  sessionStorage.setItem('sm_session_started_at', String(sessionStartedAt));
  ...
  let formOpenedAt = now;

  function endpointReady() {
    return CONFIG.endpoint.startsWith('https://script.google.com/macros/s/') &&
           CONFIG.endpoint.endsWith('/exec');
  }
  ...
  function transmit(payload, preferBeacon = false) {
    if (!endpointReady()) {
      if (CONFIG.debug) console.info('[SponsorMohawk analytics not configured]', payload);
  ...
  function track(eventName, detail = '', extra = {}, preferBeacon = false) {
    ...
    storage.set('sm_last_seen', String(lastActivityAt));
  ...
  async function collectNetworkIntel() {
    ...
    try {
      const threatRes = await fetch(
        CONFIG.threatLookupBase + encodeURIComponent(intel.publicIp),
        { cache: 'no-store' }
      );
  ...
  function setupFormTracking() {
    ...
    document.addEventListener('input', (event) => {
      if (!formStarted && event.target.closest(`#${FORM_ID}`)) {
        formStarted = true;
        formOpenedAt = Date.now();
        track('form_started', 'Sponsor partnership inquiry');
      }
    }, { passive: true });
    ...
    form.addEventListener('submit', async (event) => {
      ...
      const formElapsedSeconds = Math.max(0, Math.round((Date.now() - formOpenedAt) / 1000));

      if (honeypot) {
        await track('honeypot_triggered', 'Sponsor inquiry honeypot filled', { formElapsedSeconds });
        return;
      }
      ...
      if (urlMatches.length > 3) {
        await track('form_blocked_url', 'Message contained excessive URLs', { formElapsedSeconds });
        return;
      }

      const submitButton = form.querySelector('[type="submit"]');
      const formNote = document.getElementById(FORM_NOTE_ID);
      ...
      const payload = basePayload('form_submitted', 'Sponsor partnership inquiry', {
        contactName: String(data.get('name') || '').trim(),
        contactCompany: String(data.get('company') || '').trim(),
        contactEmail: String(data.get('email') || '').trim(),
        contactPhone: String(data.get('phone') || '').trim(),
        contactInterest: String(data.get('interest') || '').trim(),
        contactMessage: message.trim(),
        formElapsedSeconds
      });

      const sent = await transmit(payload);
      ...
      if (sent) {
        ...
        formOpenedAt = Date.now();
        if (formNote) {
          formNote.textContent = 'Thanks — your inquiry has been sent to Team Mohawk. We’ll be in touch.';
        }
        if (submitButton) {
        ...
        if (formNote) formNote.textContent = 'Something went wrong. Please try again.';
        if (submitButton) {
          submitButton.disabled = false;
          submitButton.textContent = originalText;
        }
  ...
  document.addEventListener('click', (event) => {
    ...
    const label =
      ...
      target.href;
    track('link_click', label, { destinationUrl: target.href }, true);
  }, true);
  ...
  document.addEventListener('visibilitychange', () => {
    track(
      ...
      document.hidden ? 'Tab hidden or page left' : 'Tab visible again',
      {},
      true
    );
  });
  ...
  window.SMAnalytics = { track, visitorId, sessionId, intel };

  (async () => {
    await collectNetworkIntel();
    track('page_view', 'Viewed SponsorMohawk page', {
      firstVisitThisBrowser: !storage.get('sm_seen_before')
    });
    storage.set('sm_seen_before', '1');
    setupFormTracking();
  })();
})();
